...
Your task is to create an algorithm to convert a series of ones and zeroes from the scanner into an Universal Product Code (UPC).
You can learn more about UPC from Wikipedia.
We will be using the UPC-A formatting.
...
Each item is described using the pattern below.
The number indicates how many modules wide, the letter is the colour of the bar: W for white and B for black.
...
Your task:
Your function will receive a string consisting of ones (black) and zeroes (white), and should return the UPC as a string.
Each one or zero will correspond to one module of width.
Only valid barcodes will be supplied, and they will always be presented from left to right.
