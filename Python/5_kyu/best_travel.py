...
def choose_best_sum(t, k, ls):
    # Если количество доступных элементов меньше k, вернуть None
    if len(ls) < k:
        return None
    
    # Находим все возможные комбинации k элементов
    ...
    for combination in combinations:
        ...
        # Если текущая сумма меньше или равна t, и она больше, чем текущий лучший результат
        if current_sum <= t and (best_sum is None or current_sum > best_sum):
            best_sum = current_sum
    
    # Если не было найдено подходящей суммы, возвращаем None
    return best_sum
