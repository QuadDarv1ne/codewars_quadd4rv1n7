...
def josephus(items,k):
    ...
    # Continue until all items are eliminated
    while items:
        # Calculate the index of the next item to eliminate
        index = (index + k - 1) % len(items)
        result.append(items.pop(index))  # Remove the item and add it to the result
